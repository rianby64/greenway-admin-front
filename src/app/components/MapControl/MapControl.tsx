...
import { TileLayer, FeatureGroup, useMap, Polyline } from "react-leaflet";
...
export const MapControl: React.FunctionComponent = () => {
  const map = useMap();
  const [currentMapLayer, setCurrentMapLayer] = React.useState<string>(
    MapLayers.OSM.name
  );
  const { polilines } = useTypedSelector((store) => store.route);
  const dispatch = useDispatch();
  const currentFeatureDispatcher = (e) => {
    if (e.target instanceof L.Marker)
      dispatch(setCurrentFeature(e.target.getLatLng()));
    if (e.target instanceof L.Polyline)
      dispatch(setCurrentFeature(e.target.getLatLngs()[0]));
  };
    ...
    map.eachLayer((layer) => {
      if (layer instanceof L.TileLayer) {
        ...
        if (currentMapLayer === MapLayers.OSM.name) {
          const newTileLayer = new L.TileLayer(
            MapLayers.Sattelite.mapLayersUrl,
            { attribution: MapLayers.Sattelite.mapAttribution }
          );
          map.addLayer(newTileLayer);
          setCurrentMapLayer(MapLayers.Sattelite.name);
        } else {
          const newTileLayer = new L.TileLayer(MapLayers.OSM.mapLayersUrl, {
  ...
  return (
    <>
      <FeatureGroup>
        ...
        />
        {polilines.length && <Polyline positions={polilines} />}
      </FeatureGroup>
      <TileLayer
        attribution={MapLayers.OSM.mapAttribution}
        url={MapLayers.OSM.mapLayersUrl}
      />
      <Styled.Button className="switch-tiles" onClick={switchLayer}>
        Изменить тип карты
      </Styled.Button>
      <Styled.Input type="file" className="open-file" onChange={readFile} />
    </>
  );
};
